...
#[poise::command(
    slash_command,
    rename = "kick",
    description_localized("en-US", "Kick a member from the guild"),
    guild_only,
    default_member_permissions = "MANAGE_GUILD"
)]
pub async fn kick(
    ctx: InteractionContext<'_>,
    #[description = "Whom to kick?"] member: Member,
    #[description = "What is the reason?"] reason: String,
    #[description = "Can this member appeal this kick?"]
    #[min = 1]
    appeal: Option<bool>,
    ...
    if let Ok(dm_channel) = member.user.create_dm_channel(&ctx.http()).await {
        dm_channel
            ...
            .title(format!("Kicked from {}", guild_id.name(&ctx).unwrap()))
            .description(format!("Your access to this guild has been revoked due to the following reason:\n\n**{}**", &reason))))
        .await?;
    }
    ...
    match member.kick_with_reason(&ctx.http(), &reason).await {
        Ok(_) => {
            let message = &format!("{} has been kicked due to: {}", member.user.name, &reason);
